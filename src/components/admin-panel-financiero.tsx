...
export function AdminPanelFinanciero() {
  ...
  const cuotasPagadas = s.tratamientos.flatMap((t) =>
    t.cuotas
      .filter((c) => c.estado === "Pagado")
      .map((c) => ({ monto: c.monto, fecha: c.vencimiento, tratamiento: t.nombre })),
  );
  const cuotasPend = s.tratamientos.flatMap((t) =>
    t.cuotas.filter((c) => c.estado !== "Pagado").map((c) => c.monto),
  );

  const ingMes = cuotasPagadas
    .filter(
      (c) =>
  ...
  const dataPie = cats.map((c) => ({
    name: c,
    value: s.gastos.filter((g) => g.categoria === c).reduce((a, g) => a + g.monto, 0),
  }));
    ...
    <>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-border bg-card">
        <div className="border-b border-border p-4 font-display text-lg font-bold">
          Top 5 tratamientos por ingresos
        </div>
            ...
            <tbody className="divide-y divide-border">
              {topTrat.map((t) => (
                <tr key={t.id}>
                  ...
                  <td className="p-3">{s.pacientes.find((p) => p.id === t.pacienteId)?.nombre}</td>
                  <td className="p-3">
                    {s.profesionales.find((p) => p.id === t.profesionalId)?.nombre}
                  </td>
                  <td className="p-3 text-right font-semibold">{formatCLP(t.costoTotal)}</td>
                </tr>
              ))}
            </tbody>
          </table>
