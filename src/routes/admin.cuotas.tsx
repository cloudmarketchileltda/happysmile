...
function AdminCuotas() {
  ...
  const planes = s.tratamientos.map((t) => {
    ...
    return {
      ...
      pag,
      prox,
      estado: pag === t.cuotas.length ? "Pagado" : tieneMora ? "Con mora" : "Al día",
    };
  });
              ...
              <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    {pag}/{t.cuotas.length} cuotas · {formatCLP(t.costoTotal)} total · próx. vence{" "}
                    {prox ? formatDate(prox.vencimiento) : "—"}
                  </div>
                </div>
              </div>
                ...
                {t.cuotas.map((c) => (
                  ...
                  >
                    ...
                    {c.estado !== "Pagado" && (
                      <button
                        onClick={() => {
                          s.marcarCuotaPagada(t.id, c.numero);
                          toast.success(`Cuota #${c.numero} pagada`);
                        }}
                        className="rounded bg-brand px-2 py-0.5 text-[10px] font-semibold text-brand-foreground"
                      >
                        Marcar pagada
                      </button>
                    )}
