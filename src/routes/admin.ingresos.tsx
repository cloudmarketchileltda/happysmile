...
function AdminIng() {
  const s = useClinic();
  const [fProf, setFProf] = useState(""); const [fEsp, setFEsp] = useState("");
  ...
  const hoy = new Date();
  const ingMes = s.tratamientos.flatMap((t) => t.cuotas).filter((c) => c.estado === "Pagado" && new Date(c.vencimiento).getMonth() === hoy.getMonth() && new Date(c.vencimiento).getFullYear() === hoy.getFullYear()).reduce((a, c) => a + c.monto, 0);
  const proyectado = s.tratamientos.flatMap((t) => t.cuotas).filter((c) => c.estado === "Pendiente").reduce((a, c) => a + c.monto, 0);
  const mora = s.tratamientos.flatMap((t) => t.cuotas).filter((c) => c.estado === "Vencido").reduce((a, c) => a + c.monto, 0);
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard label="Ingreso proyectado" value={formatCLP(proyectado)} tone="brand" hint="Suma de cuotas pendientes" />
