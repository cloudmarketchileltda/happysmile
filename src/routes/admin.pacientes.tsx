...
function AdminPac() {
  ...
  const filtered = s.pacientes.filter(
    (p) => p.nombre.toLowerCase().includes(q.toLowerCase()) || p.rut.includes(q),
  );
  ...
  const save = () => {
    ...
    if (editId) {
      s.updatePaciente(editId, form);
      toast.success("Paciente actualizado");
    } else {
      s.addPaciente({
        ...form,
        ficha: form.ficha || "HS-" + String(s.pacientes.length + 1).padStart(4, "0"),
      });
