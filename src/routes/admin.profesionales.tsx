...
function AdminProf() {
  ...
  const openEdit = (id: string) => {
    const p = s.profesionales.find((x) => x.id === id)!;
    setEditId(id);
    setForm(p);
    setOpen(true);
  };
  const save = () => {
    if (!form.nombre || !form.especialidadId) {
      toast.error("Completa los campos obligatorios");
      return;
    }
    if (editId) {
      s.updateProfesional(editId, form);
      toast.success("Profesional actualizado");
    } else {
      s.addProfesional(form);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Mantenedor de especialistas"
        action={
          <button
            onClick={openNew}
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((p) => (
                <tr key={p.id}>
                  ...
                  <td className="p-3">
                    {s.especialidades.find((e) => e.id === p.especialidadId)?.nombre}
                  </td>
      ...
      <Modal
        ...
        title={editId ? "Editar profesional" : "Nuevo profesional"}
        size="lg"
        footer={
          <>
            <button
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Field label="Credenciales">
            <input
              ...
              onChange={(e) => setForm({ ...form, credenciales: e.target.value })}
            />
          </Field>
        </div>
      </Modal>
