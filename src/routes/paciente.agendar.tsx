            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {s.especialidades.map((e) => (
                <button
                  ...
                  className={
                    "rounded-xl border p-4 text-left transition-all hover:border-brand " +
                    (espId === e.id ? "border-brand bg-brand-soft" : "border-border")
                  }
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{e.duracionMin} min</div>
                    </div>
                  </div>
                </button>
              ))}
            ...
            <div className="grid gap-3">
              {profsDisp.map((p) => (
                <button
                  ...
                  className="flex items-center gap-3 rounded-xl border border-border p-3 text-left hover:border-brand"
                >
                  <div className="grid h-12 w-12 place-items-center rounded-full bg-brand text-sm font-bold text-brand-foreground">
                    {p.nombre
                      .split(" ")
                      .slice(0, 2)
                      .map((n) => n[0])
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">{p.horario}</div>
                  </div>
                </button>
              ))}
            </div>
              ...
              {proximaDias.map((d) => (
                ...
                >
                  ...
                  <div className="text-muted-foreground">
                    {new Date(d).toLocaleDateString("es-CL", { weekday: "short" })}
                  </div>
                </button>
              ))}
            </div>
        ...
        {paso === 5 && (
          <div>
            ...
            <div className="rounded-xl bg-brand-soft p-4">
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-semibold">{esp?.nombre}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Profesional</dt>
                  <dd className="font-semibold">{prof?.nombre}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Fecha</dt>
                  <dd className="font-semibold">{fecha && formatDate(fecha)}</dd>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="font-semibold">{esp?.duracionMin} min</dd>
                </div>
              </dl>
            </div>
            <div className="mt-4 flex gap-2">
              ...
              <button
                onClick={confirmar}
                className="flex-1 rounded-lg bg-brand py-2 text-sm font-semibold text-brand-foreground hover:opacity-90"
              >
                Confirmar hora
              </button>
            </div>
        ...
        {paso === 6 && (
          <div className="py-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Te enviamos la confirmación por correo. Te esperamos el {fecha && formatDate(fecha)} a
              las {hora}.
            </p>
            <button
              onClick={() => {
              ...
              className="mt-6 rounded-lg bg-brand px-6 py-2 text-sm font-semibold text-brand-foreground"
            >
              Agendar otra hora
            </button>
          </div>
