...
function PacienteHome() {
  ...
  const saldoPendiente = tratamientos.reduce(
    (acc, t) =>
      acc + t.cuotas.filter((c) => c.estado !== "Pagado").reduce((a, c) => a + c.monto, 0),
    0,
  );
  const totalPagado = tratamientos.reduce(
    (acc, t) =>
      acc + t.cuotas.filter((c) => c.estado === "Pagado").reduce((a, c) => a + c.monto, 0),
    0,
  );
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={formatCLP(saldoPendiente)}
          tone={saldoPendiente > 0 ? "coral" : "default"}
        />
        <StatCard
          label="Próximas citas"
          value={String(
            s.citas.filter((c) => c.pacienteId === paciente.id && new Date(c.fechaISO) > new Date())
              .length,
          )}
          tone="default"
        />
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          {proxima ? (
            <div>
              ...
              <Link
                to="/paciente/horas"
                className="mt-4 inline-flex rounded-lg border border-border px-4 py-2 text-sm hover:bg-muted"
              >
                Ver todas mis horas
              </Link>
            </div>
            ...
            <div className="text-sm text-muted-foreground">
              ...
              <Link to="/paciente/agendar" className="font-semibold text-brand">
                Agendar ahora →
              </Link>
            </div>
          )}
          ...
          {activo ? (
            <div>
              ...
              <div className="mt-1 text-sm text-muted-foreground">
                {s.especialidades.find((e) => e.id === activo.especialidadId)?.nombre}
              </div>
              <div className="mt-4">
                <div className="mb-1 flex justify-between text-xs">
                  <span>Avance</span>
                  <span>
                    {activo.etapas.filter((e) => e.completada).length}/{activo.etapas.length} etapas
                  </span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-muted">
                  <div
                    ...
                    style={{
                      width: `${(activo.etapas.filter((e) => e.completada).length / activo.etapas.length) * 100}%`,
                    }}
                  />
                </div>
              </div>
