...
function ModificarPage() {
  ...
  const proximas = s.citas
    .filter(
      (c) =>
        c.pacienteId === s.pacienteActualId &&
    ...
    <>
      ...
      {proximas.length === 0 ? (
        <EmptyState
          ...
          title="No tienes citas modificables"
        />
      ) : (
              ...
              >
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      setEditId(c.id);
                      setNuevaFecha(c.fechaISO.slice(0, 10));
                      setNuevaHora(formatTime(c.fechaISO));
                    }}
                  ...
                  <button
                    ...
                    className="rounded-lg bg-coral px-3 py-2 text-sm font-semibold text-coral-foreground hover:opacity-90"
                  >
                    Cancelar
                  </button>
                </div>
      ...
      <Modal
        ...
        description={
          editingCita
            ? `${formatDate(editingCita.fechaISO)} · ${formatTime(editingCita.fechaISO)}`
            : ""
        }
        footer={
          <>
      ...
      >
        <p className="text-sm text-muted-foreground">
          Si necesitas reagendar, puedes hacerlo desde "Agendar Hora".
        </p>
      </Modal>
    </>
  );
