...
function AgendaProf() {
  ...
  const fechas = Array.from({ length: dias }).map((_, i) => {
  ...
  s.citas
    .filter((c) => c.profesionalId === prof.id)
    .forEach((c) => {
  ...
  return (
    <>
          ...
          <div className="flex gap-1 rounded-lg bg-muted p-1">
            <button
              ...
              className={
                ...
                (vista === "semana" ? "bg-background shadow-sm" : "text-muted-foreground")
              }
            >
              Semana
            </button>
            <button
              ...
              className={
                "rounded-md px-3 py-1 text-sm font-medium " +
                (vista === "mes" ? "bg-background shadow-sm" : "text-muted-foreground")
              }
            >
              Mes
            </button>
      ...
      <div
        className={
          "grid gap-2 " + (vista === "semana" ? "sm:grid-cols-7" : "sm:grid-cols-5 lg:grid-cols-7")
        }
      >
        {fechas.map((d) => {
          const k = d.toISOString().slice(0, 10);
          const items = (citasMap.get(k) ?? []).sort(
          ...
          return (
            <div
              key={k}
              className={
                "rounded-xl border bg-card p-2 " + (isHoy ? "border-brand" : "border-border")
              }
            >
              <div className="mb-2 text-center text-xs">
                ...
                <div className={"font-display text-lg font-bold " + (isHoy ? "text-brand" : "")}>
                  {d.getDate()}
                </div>
              </div>
                ...
                {items.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full rounded bg-brand-soft p-1.5 text-left hover:bg-brand-soft/70"
                    >
                      <div className="text-[10px] font-bold text-brand">
                        {formatTime(c.fechaISO)}
                      </div>
                      <div className="truncate text-[11px] font-medium">{p?.nombre}</div>
                      <div className="truncate text-[10px] text-muted-foreground">
                        {c.duracionMin} min · {c.box}
                      </div>
                    </button>
                  );
                })}
        ...
        {cita && (
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{pac?.nombre}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {formatDate(cita.fechaISO)} · {formatTime(cita.fechaISO)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Duración</span>
              <span className="font-semibold">{cita.duracionMin} min</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Box</span>
              <span className="font-semibold">{cita.box}</span>
            </div>
            <div className="flex justify-between">
