...
function PerfilProf() {
  ...
  const [disp, setDisp] = useState({ lun: true, mar: true, mie: true, jue: true, vie: true, sab: false, dom: false });

  const dias = [["lun", "Lunes"], ["mar", "Martes"], ["mie", "Miércoles"], ["jue", "Jueves"], ["vie", "Viernes"], ["sab", "Sábado"], ["dom", "Domingo"]] as const;

  const guardar = () => { s.updateProfesional(prof.id, form); toast.success("Perfil actualizado"); };
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
        <div className="rounded-2xl border border-border bg-card p-6 text-center">
          <div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-brand text-2xl font-bold text-brand-foreground">{prof.nombre.split(" ").slice(0,2).map((n) => n[0]).join("")}</div>
        ...
        <div className="space-y-4">
          <div className="grid gap-4 rounded-2xl border border-border bg-card p-6 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2"><Field label="Biografía"><textarea rows={3} className={inputCls} value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} /></Field></div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {dias.map(([k, label]) => (
                <label key={k} className="flex items-center justify-between rounded-lg border border-border bg-background p-3 text-sm">
                  <span>{label}</span>
                  <button onClick={() => setDisp({ ...disp, [k]: !disp[k] })} className={"h-6 w-11 rounded-full transition-colors " + (disp[k] ? "bg-brand" : "bg-muted")}>
                    <span className={"block h-5 w-5 transform rounded-full bg-white transition-transform " + (disp[k] ? "translate-x-6" : "translate-x-0.5")} />
                  </button>
                </label>
              ))}
            </div>
